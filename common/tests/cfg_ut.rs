...
mod cfg_v1_ut_oci {
    ...
    fn test_cfg_v1_pdm_exported_app_path() {
        ut_rt::tb("cfg-v1/podman.yaml".to_string(), |cfg| {
            let cfg = cfg.unwrap();
            assert!(cfg.runtime().paths().iter().next().unwrap().0 == &PathBuf::from("/usr/bin/banana"));
        });
    }
    ...
    fn test_cfg_v1_pdm_registered_app_path() {
        ut_rt::tb("cfg-v1/podman.yaml".to_string(), |cfg| {
            let cfg = cfg.unwrap();
            assert!(
                cfg.runtime().paths().iter().next().unwrap().1.clone().exports() == &PathBuf::from("/banana/in/the/container")
            );
        });
    }
...
mod cfg_v1_ut_vm {
    ...
    fn test_cfg_v1_vm_target_app_path() {
        ut_rt::tb("cfg-v1/firecracker.yaml".to_string(), |cfg| {
            let cfg = cfg.unwrap();
            assert!(cfg.runtime().paths().iter().next().unwrap().0 == &PathBuf::from("/usr/sbin/hell"));
        });
    }
    ...
    fn test_cfg_v1_vm_host_app_path() {
        ut_rt::tb("cfg-v1/firecracker.yaml".to_string(), |cfg| {
            let cfg = cfg.unwrap();
            assert!(cfg.runtime().paths().iter().next().unwrap().1.clone().exports() == &PathBuf::from("/highway/to/hell"));
        });
    }
